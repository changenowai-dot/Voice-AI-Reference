...
class VoiceOverApp(tk.Tk if tk else object):        # noqa: D101
    ...
    def _build_ui(self):
        ...
        ttk.Radiobutton(lang_card, text="Deutsch", value="German",
                        ...
                        command=self._update_stats).pack(side=LEFT, padx=12,
                                                         pady=8)
        ...
        # STIMME
        voice_card = ttk.Labelframe(container, text=" Stimme ")
        voice_card.pack(fill=X, pady=(0, 8))
        self.voice_var = tk.StringVar(value="vd_e")
        male = ttk.Frame(voice_card)
        male.pack(fill=X, padx=8, pady=(6, 2))
        ttk.Label(male, text="Männlich", style="Muted.TLabel").pack(
            anchor=W)
        for e in sorted(self.registry.entries(), key=voice_sort_key):
            if e.gender != "male":
                continue
            self._add_voice_button(male, e)
        female = ttk.Frame(voice_card)
        female.pack(fill=X, padx=8, pady=(2, 8))
        ttk.Label(female, text="Weiblich", style="Muted.TLabel").pack(
            anchor=W)
        for e in sorted(self.registry.entries(), key=voice_sort_key):
            if e.gender != "female":
                continue
            self._add_voice_button(female, e)
    ...
    def _add_voice_button(self, parent, entry):
        label = entry.display_name
        if entry.voice_id == "vd_e":
            label = "VD-E  –  Standard (empfohlen)"
        elif entry.recommended:
            label += "  – empfohlen"
        btn = ttk.Radiobutton(parent, text=label, value=entry.voice_id,
                              variable=self.voice_var)
        btn.pack(side=LEFT, padx=(0, 18))
        if entry.available is False:
            btn.state(["disabled"])
            ttk.Label(parent,
                      text=f"{entry.display_name} (nicht verfügbar)",
                      style="Muted.TLabel").pack_forget()
    ...
    def start_job(self):
        ...
        if voice_id == "vd_e" and not self.identity.ok:
            ...
            return
        entry = self.registry.get(voice_id)
