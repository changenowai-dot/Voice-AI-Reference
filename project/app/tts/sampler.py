...
# Produktion (config/production.json): Headroom 5,0 s – als Default
# übernommen (§3 LOCKED PRODUCTION SETTINGS).
DEFAULT_HEADROOM_S = 5.0


def max_new_tokens_for(seconds: float, headroom_s: float | None = None) -> int:
    """Begrenzt die Generierung (verhindert Endlosschleifen/Runaway)."""
    hr = DEFAULT_HEADROOM_S if headroom_s is None else headroom_s
